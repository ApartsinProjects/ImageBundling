...
concurrency observation (Section~5.5) should be read; none of it
...
whole-bundle cache-invalidation problem (Section~6.2). HTTP
...
A live-renderer measurement (Section~5.6) confirms that at typical tile
...
Section~5.4 shows two encoder flags reverse it to +5\%. Edge-replicated
...
benefit is cache granularity and bounded invalidation (Section~6.2,
Figure~4), not packet-loss resilience. The byte-bundle beats individual
...
\subsection{Local rendering cost: decode time and
memory}
...
isolate rendering (Table~7). We render N 96-pixel photographic tiles in
...
that baseline (Table~8), and it fails even on the isolated codec
...
set. This is the empirical case for the design of Section~6.3: the right
...
\subsection{Cache invalidation and delta
updates}
...
trade-off explicit (Figure~4). Serving immutable individual files is the
...
(Figure~5). For a lossy group the routing is not decided by tile size
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.62\textwidth]{fig/fig5.pdf}
...
deltas. This is the procedure evaluated against the oracle in Table~9.}
...
the byte-optimal one (Table~9). Each group carries one measured
...
The value is in choosing, not in any single layout. Table~9 also scores
...
\subsection{Choosing a rendering
mechanism}
...
Section~6.3 chooses the byte layout; this table chooses the display
...
control, and support. Pick the byte layout with the Section~6.3 selector
...
renderer (Section~3.2, Table~7), confirming the pixel atlas is
...
server's own QUIC transport log (Section~5.5) and
...
further independent corpora (Section~6.3); a still wider survey of
...
with bootstrap confidence intervals (Table~6) but no formal significance
...
renderer (Table~7) but LCP, decode CPU, and warm-cache multi-navigation
...
Section~5.5) rather than a protocol-general result. The unifying
